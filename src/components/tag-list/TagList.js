...
const SvgButton = ({ onClick, background, svgColor }) => {
  return (
    <button
...
const SvgButtonContainer = ({ onClick }) => {
    const backgrounds = [
        "rgba(213, 240, 219, 1)",
        "rgba(213, 235, 240, 1)",
        "rgba(255, 255, 143, 1)",
  ...
  const handleButtonClick = (background) => {
    // Handle button click
    console.log("Button clicked with background:", background);
    ...
    <div className="svg-button-container">
      {backgrounds.map((background, index) => (
        <SvgButton
          ...
          svgColor={svgColors[index]} // Передача цвета SVG изображения
        />
      ))}
    </div>
  );
